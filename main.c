...
unsigned char pdata bufRxd[16],cntRxd=0;
unsigned long xdata PeriodCnt=0;
unsigned char xdata HighRH=0;
unsigned char xdata HighRL=0;
unsigned char xdata LowRH=0;
unsigned char xdata LowRL=0;
...
unsigned char code str13[]="7 PWMOUT";
unsigned char code str14[]="Duty cycle";
...
unsigned char xdata ircode[4],addr=10,m=11;
...
bit en1602,en16022,enui,entimer,ds18flags,irflag,t=0,e2,rw,pwm,PWMOUT;
...
void main()
{
	...
	while(1)
	{
		...
		switch(i)//用于主界面显示，加入变量en1602和en16022是为了是程序只初始化一次，运行界面只显示一次
		{
			...
			case 7:if(en1602) {LcdAreaClean(0,1,16); LcdShowStr(0,1,str13); en1602=0;}break;
		}
		if(enui)
		switch(i)//用于启动不同模块
		{
			...
			case 4:if(en16022) {LcdAreaClean(0,0,16); LcdShowStr(0,0,str4); en16022=0;configuart();} UARTdriver(); break;
			...
			case 7:if(en16022) {LcdAreaClean(0,0,16); LcdShowStr(0,0,str13); en16022=0;ConfiPWM(100,10);pwm=1;} pwmou(); break;
		}
		keydriver();
	}
	
}
void UARTdriver()//下次添加左右一瓶
{
	if(ds18flags)//一秒以后如果没有接受数据则开始显示
	{
		if(cntRxd!=0)
		{
			LcdAreaClean(0,1,16);
...
void keyaction(unsigned char keycode)
{
		...
		if(e2==1)//EEPROM专用键盘
	{
		...
		if((ds18flags==1)&&(rw==1)&&(m==11))//写入m和写入addr延时一秒，否则会同时写入造成地址和数据值永远一样
		{
				switch(keycode)
			{
				case 1:m=1;break;
...
void interruptuart() interrupt 4
{
	...
	{
		RI=0;
		ds18flags=0;//每接收到一个字节数据，一秒计时标志清零
		bufRxd[cntRxd++]=SBUF;
		index=0;//每接收到一个字节数据，一秒计时从头开始
	}
	if(TI)
	{
		TI=0;
